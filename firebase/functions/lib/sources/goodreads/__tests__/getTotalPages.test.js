...
describe('getTotalPages', () => {
  it('returns the highest numeric page when multiple page links exist', () => {
    ...
    expect((0, utils_1.getTotalPages)($)).toBe(3);
  });
  it('returns 0 when no content exists', () => {
    ...
    `;
    const $ = cheerio.load(html);
    expect((0, utils_1.getTotalPages)($)).toBe(0);
  });
  it('handles non-numeric links correctly', () => {
    ...
    `;
    const $ = cheerio.load(html);
    expect((0, utils_1.getTotalPages)($)).toBe(2);
  });
});
//# sourceMappingURL=getTotalPages.test.js.map
