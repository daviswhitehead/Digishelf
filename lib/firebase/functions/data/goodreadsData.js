...
async function getPageItems(baseURL, pageNumber) {
    ...
    try {
        ...
        const books = await Promise.all(bookRows.map(elem => limit(async () => {
            ...
            // --- Rating ---
            const rating = translateRating($(elem).find('td.field.rating span.staticStars').attr('title')) || null;
            // --- Review ---
        ...
        if (totalPages > 1) {
            ...
            results.forEach((result, index) => {
                if (result.status === 'fulfilled') {
                    ...
                    console.error(`Failed to fetch page ${index + 2} (${originalURL}&page=${index + 2}):`, result.reason);
                }
            });
        }
...
//# sourceMappingURL=goodreadsData.js.map